...
class HELMMultiViewRouter(nn.Module):
    ...
    def forward(self, hidden_states, current_step = None):
        ...
        # Multiply this by Tau (router_init_scale) and ngpt scaler sqrt(hidden_size)
        ...
        # convert step value into tensor (attach to device too)
        # Goal: Get mask to size [batch_size, 1, num_attention_heads - num_permanent_heads]
        step_tensor = torch.tensor(step, device=sigmoid_scores.device)
        ...
        if self.training:
            ...
            # Very Simple: Multiply set scaling factor (sparsity_lamdba) by the mean of the sigmoid_scores (remember sigmoid scores don't add up to 1)
            self.sparsity_loss = self.config.sparsity_lambda * sigmoid_scores.mean()
...
class HELMModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, current_step = None):
        ...
        for block in self.blocks:
            hidden_states, aux_loss, sparsity_loss = block(hidden_states, attention_mask, current_step)
